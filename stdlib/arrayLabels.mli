...
val copy : ('a : value_or_null mod separable). 'a array -> 'a array
...
val fold_left : ('a : value_or_null mod separable).
   f:('acc -> 'a -> 'acc) -> init:'acc -> 'a array -> 'acc
...
val fold_right : ('a : value_or_null mod separable).
   f:('a -> 'acc -> 'acc) -> 'a array -> init:'acc -> 'acc
...
val for_all2 : ('a : value_or_null mod separable).
   f:('a -> 'b -> bool) -> 'a array -> 'b array -> bool
...
val mem : ('a : value_or_null mod separable). 'a -> set:'a array -> bool
(** [mem a ~set] is true if and only if [a] is structurally equal
    to an element of [set] (i.e. there is an [x] in [set] such that
    [compare a x = 0]).
    @since 4.03 *)

val memq : ('a : value_or_null mod separable). 'a -> set:'a array -> bool
...
val find_mapi : ('a : value_or_null mod separable).
   f:(int -> 'a -> 'b option) -> 'a array -> 'b option
