...
let of_host_int_exn i =
  ...
  | 63, Thirty_two ->
    if i >= -0x8000_0000 && i <= 0x7fff_ffff
    then Thirty_two (Int32.of_int i)
...
let to_int64 t =
  match t with Thirty_two t -> Int64.of_int32 t | Sixty_four t -> t
...
let width_as_int64 () =
  match zero () with Thirty_two _ -> 4L | Sixty_four _ -> 8L
