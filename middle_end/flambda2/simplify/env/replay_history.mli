...
(** {1 Replay Histories}
    ...
    interested in doing multiple downwards pass on a single term. In such cases,
    it might be necessary to relate the names of variables and continuations
    from the first pass to those from the subsequent passes. The predominant use
    of that is currently to correctly handle continuations that have been lifted
    during the first pass, and whose calls should be rewritten in subsequent
    passes (including their lifted arguments).

    This module provides a way to "replay" a downwards pass and more
    specifically its sequence of bindings, so that the names of continuations
    and variables generated during the second pass can be mapped to the names
    generated during the first pass.

    As a safety measure, before mapping two names, we check that they are in the
    same "renaming equivalence class", though note that this is not a guarantee
    (for isntance, by default all continuation have no names and therefore will
...
(** The type of a replay history, used both when recording during the first
    downwards pass **and** when replaying during a subsequent downwards pass on
    the same term. *)
type t
...
(** Given a history generated during a first downwards pass on an expression
    [expr], create a new history that is suitable for use for subsequent passes
    on [expr]. *)
...
(** Returns [true] is the replay history was created with the `always_inline`
    flag. (first pass histories have the flag set to false). *)
val must_inline : t -> bool

(** Type of results for inspection functions for which the result only makes
    sense when replaying a downwards pass. *)
type 'a replay_result =
  | Still_recording
  | Replayed of 'a

(** If called on a history created with {!replay}, returns the mapping from
    variables of the current pass to variables bound during the first pass. *)
val replay_variable_mapping : t -> Variable.t Variable.Map.t replay_result

(** If called on a history created with {!replay}, returns the mapping from
    continuations of the current pass to continuations bound during the first
    pass. *)
