...
(** {1 Continuation Lifting Status}
    ...
    Continuation lifting is done in simplify on the way down. Considering a term
    of the form:
...
(** The decision to lift continuations (e.g. k') out of another (e.g. k), is
    made once Simplify has reached the bottom of the handler of k **but** has
    not yet explored the handler of k'. The [t] type represente the current
    ...
    - it is set to [Analyzing] when going downwards through the handler of a
      continuation.
    - when we make the choice of whether to lift or not[1], if we do decide to
      lift, we set it to [Lifting_out_of].
